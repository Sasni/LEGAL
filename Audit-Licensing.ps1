...
if ($retailWinLic.Count -gt 0 -and [string]::IsNullOrWhiteSpace($oemKey)) {
    ...
    # Check ACPI MSDM table existence — more reliable than BIOS date
    $biosDate = try { (Get-CimInstance -ClassName Win32_BIOS -ErrorAction Stop).ReleaseDate } catch { $null }
    $msdmExists = $true
    try {
        $null = Get-CimInstance -Namespace root/wmi -ClassName MSDM_PhysicalDevices -ErrorAction Stop
    } catch {
        $msdmExists = $false
    }
    $isLegacySystem = (-not $msdmExists) -or ($biosDate -and $biosDate -lt (Get-Date '2012-10-26'))

    if ($isLegacySystem) {
        Add-Finding -Id "HWID_RETAIL_NO_OEM_KEY" -Severity "Info" -Area "MAS HWID" `
            -Evidence "Windows pokazuje kanał Retail i status Licensed, ale nie znaleziono klucza OEM w firmware. Tabela MSDM nie istnieje w ACPI — typowe dla maszyn z ery Windows 7 lub składaków (BIOS: $($biosDate.ToString('yyyy-MM-dd')))." `
